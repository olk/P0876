...
\begin{itemize}
    ...
    \item Added definition of ``processor thread''
          in \stdclause{intro.progress} paragraph 8, using the words already
          present, to be able to reference that term elsewhere.
    \item Clarified that \cpp{thread\_local} storage is per processor thread.
    \item Clarified that a blocking operation blocks a processor thread.
    \item Clarified that ``potentially concurrent'' actions are performed by
    ...
    \item Added \cpp{fiber\_context::current\_exception\_within\_fiber}.
