...
\begin{tabular}{ l l }
    ...
    \cpp{template<typename Fn>}\\
    \cpp{fiber resume\_with(Fn&& fn) &} & (3)\\

    \midrule

    \cpp{template<typename Fn>}\\
    \cpp{fiber resume\_with(Fn&& fn) &&} & (4)\\

    \midrule
\end{tabular}

\begin{description}
    \item[1)] suspends the active fiber, resumes fiber \cpp{*this}; lvalue-ref qualified
    \item[2)] suspends the active fiber, resumes fiber \cpp{*this}; rvalue-ref qualified
    \item[3)] suspends the active fiber, resumes fiber \cpp{*this}
              but calls \cpp{fn()} in the resumed fiber (as if called by the
              suspended function); lvalue-ref qualified
    \item[4)] suspends the active fiber, resumes fiber \cpp{*this}
              but calls \cpp{fn()} in the resumed fiber (as if called by the
              suspended function); rvalue-ref qualified
\end{description}
