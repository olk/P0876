...
\cbstart
¶ When the running fiber resumes a prepared fiber (by returning
from its entry function or by calling \qualmethod{\resumewith}), all
evaluations prior to the fiber context switch are sequenced before any evaluations
subsequent to the fiber context switch in the resumed fiber's entry function. When
the running fiber resumes a suspended fiber, all evaluations prior to the fiber
context switch are sequenced before any evaluations subsequent to the fiber context
switch in the resumed fiber's call to \resumewith.
\cbend
...
one fiber to another is a \emph{fiber context switch}.
