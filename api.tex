...
\zs{Modify §14.2 \stdclause{except.throw} paragraph 2 as indicated:}
...
\zs{Modify §14.2 \stdclause{except.throw} paragraph 4 Note 3 as indicated:}
...
\zs{Modify §14.4 \stdclause{except.handle} paragraph 6 as indicated:}
...
\cpp{span<byte>} in which to track the fiber's invocation sequence
\xref{stacktrace.general}. If at any time during the life of a fiber the
data storage required to track its invocation sequence exceeds the
\cpp{size()} of that \cpp{span<byte>}, the behaviour is undefined.
...
\begin{description}
    \item[---] Let \cpp{entry\_copy} be an object of
               type \cpp{std::decay\_t<F>} direct-non-list-initialized
...
\begin{description}
    ...
    \item[---] Let \cpp{deleter\_copy} be an object of
               type \cpp{std::decay\_t<D>} direct-non-list-initialized
...
\cpp{caller} be a synthesized \fiber object representing the calling fiber.
\resumewith only returns when some other fiber resumes \cpp{caller}. At that
time, ``previous fiber'' means the fiber that resumed \cpp{caller}.
\tsnote{The previous fiber might or might not be the target fiber.}
...
\begin{description}
    ...
    \item[---] Evaluates \cpp{invoke(std::forward<Fn>(fn), std::move(caller))}.
               ...
               \tsnote{\cpp{returned} might or might not be the same as \cpp{caller}.}
               \tsnote{\cpp{returned} can be empty.}
    \item[---] If the target fiber previously suspended itself by calling one
               of \anyresume, returns \cpp{returned} from that resume function.
    \item[---] Otherwise, the target fiber has not yet been entered.
        ...
        \begin{itemize}
            ...
            \item if the target fiber has an associated \cpp{stack\_copy} and \cpp{deleter\_copy}:
                \begin{itemize}
...
\begin{description}
    \item[---] If the previous fiber resumed the calling fiber by returning a \fiber,
               an empty \fiber.
    \item[---] If the previous fiber resumed the calling fiber by passing some \cpp{fn}
...
\tsnote{When \canresume is \true, the \fiber object can be resumed
by \anyresume.}
