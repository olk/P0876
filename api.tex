...
\cbstart
13 When the running fiber resumes a prepared fiber (by returning
from its entry function or by calling \qualmethod{\resumewith}), all
evaluations prior to the context switch are sequenced before any evaluations
subsequent to the context switch in the resumed fiber's entry function. When
the running fiber resumes a suspended fiber, all evaluations prior to the
context switch are sequenced before any evaluations subsequent to the context
...
\begin{description}
    ...
    \item[---] \cpp{is\_invocable\_v<decay\_t<F>, fiber\_context&&>} is \true\\
               and \cpp{invoke\_result\_t<decay\_t<F>, fiber\_context&&>} denotes \fiber.
\end{description}
...
\begin{itemize}
    \item Invoking \resumewith performs a context switch.
    ...
    \item Because \resumewith suspends the calling fiber A, returning
          ...
          subsequent context switch back to suspended fiber A.
...
A full context switch to the target fiber requires the following steps:

\begin{enumerate}
    \item suspend the running fiber
    \item direct the current thread to make the target fiber \emph{current}:
          an \emph{internal-context-switch} to the target fiber
    \item perform execution steps on the target fiber.
\end{enumerate}
...
\tsnote{\emph{internal-resume} is executed after performing an \emph{internal-context-switch}
to the target fiber.}
...
\begin{description}
    ...
    \item[---] Otherwise:
        \begin{itemize}
            ...
            \item perform an \emph{internal-context-switch} to \thefiber{\cpp{successor}}
...
and \cpp{invoke\_result\_t<decay\_t<Fn>, fiber\_context&&>} denotes \fiber.
...
\begin{description}
    ...
    \item[---] Performs an \emph{internal-context-switch} to the target fiber.
...
\begin{description}
    \item[---] \resumewith is vectorization-unsafe \xref{algorithms.parallel.defns\#5}.
