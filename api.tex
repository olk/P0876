...
\add{\enternoten{5} It is implementation-defined whether:}
\begin{itemize}
    \item \add{\cpp{current\_exception()} returns the exception with the most
          recently activated handler that is still active}\\
          \add{within the current thread of execution, or}
    \item \add{\cpp{current\_exception()} returns the exception with the most
          recently activated handler that is still active}\\
          \add{within the current \cpp{std::thread}.}
\end{itemize}
\add{\exitnote}

\bfs{Modify} 17.9.7.12 to \emph{Note \replace{5}{6}}.

\bfs{Append} to 14.6.3.1 \stdclause{except.uncaught} as indicated:

\add{\enternoten{1} It is implementation-defined whether:}
\begin{itemize}
    \item \add{\cpp{uncaught\_exceptions()} returns the number of uncaught
          exceptions in the current thread of execution, or}
    \item \add{\cpp{uncaught\_exceptions()} returns the number of uncaught
          exceptions in the current \cpp{std::thread}.}
\end{itemize}
\add{\exitnote}
