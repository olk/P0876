...
\add{([fiber-context.general]) within the current thread. \exitnote}
...
\add{([fiber-context.general]). \exitnote}
...
\rSec2[fiber-context]{Cooperative Threads}
...
\effects
\begin{description}
    \item[---] Performs \emph{decay-copy}\cpp{(std::forward<F>(entry))}, and let the variable initialized as if thereby be referred to as \cpp{entry\_copy} throughout this clause.
...
\errors
\begin{description}
    \item[---] \cpp{resource\_unavailable\_try\_again} -- the system lacked the necessary resources to create another fiber.
\end{description}

%---------------------------- explicit stack ctor ----------------------------
\mbrhdr{template<typename F, typename D> fiber\_context(F\&\& entry, span<byte> stack, D\&\& deleter)}
...
\begin{description}
    \item[---] Performs \emph{decay-copy}\cpp{(std::forward<F>(entry))}, and let the variable initialized as if thereby be referred to as \cpp{entry\_copy} throughout this clause.
    \item[---] Performs \emph{decay-copy}\cpp{(stack)}, and let the variable initialized as if thereby be referred to as \cpp{stack\_copy} throughout this clause.
    \item[---] Performs \emph{decay-copy}\cpp{(std::forward<D>(deleter))}, and let the variable initialized as if thereby be referred to as \cpp{deleter\_copy} throughout this clause.
    \item[---] Instantiates a \fiber representing a fiber suspended before
              entry to \cpp{entry\_copy}.
              \tsnote{\cpp{entry\_copy} is entered only when \anyresume is called.}
...
\postcond
\begin{description}
    \item[---] \cpp{other.}\emptyfn returns \true
\end{description}
