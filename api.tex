...
\except
\begin{description}
    \item[---] \cpp{bad\_alloc} on failure to acquire necessary system resources.
\end{description}
...
\remarks
\begin{description}
    \item[---] On return from \cpp{Entry}, evaluates \cpp{invoke(Deleter,
               Stack)} on the fiber represented by the \cpp{fiber\_context}
               returned by \cpp{Entry}.
\end{description}
...
\returns
\begin{description}
    \item[---] \cpp{false} if \cpp{empty()} returns \cpp{true}, or if \currthread is not the same
        as \lastthread.
\end{description}
