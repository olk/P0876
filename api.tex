...
\zs{Insert before \stdsection{6.10.1}{intro.execution} paragraph 13 and
renumber existing paragraphs 13-16:}

\add{¶ Given a fiber context switch from fiber A to fiber B, the fiber context switch in A is sequenced before the fiber context}\\
\add{switch in B.}
...
%% [basic.start.main]: Executing a program starts a main thread of execution
%% (6.10.2, 32.4) in which the main function is invoked.
%% [thread.thread.class] does not talk about creating a thread!
%% [thread.thread.class.general] p1: The class thread provides a mechanism to
%% create a new thread of execution, ...
%% [thread.thread.constr] p5: Effects: The new thread of execution executes
%% invoke(...)...
%% Any return value from this invocation is ignored.
%% Same for [thread.jthread.class.general] p1, [thread.jthread.constr] p5.
...
initial invocation of a specific \emph{\entryfn}, and recursively
including every function invocation subsequently executed by the fiber. The
execution steps of a fiber are performed by a thread.
...
fiber}. Every thread starts by running a \emph{default fiber}.

3 The invocable passed to the \cpp{thread} or \cpp{jthread}
constructor \xref{thread.threads} is the \entryfn of the created thread.
The \entryfn of a thread is the \entryfn of its default fiber.

4 Constructing an object of type \stdfiber ([fiber.context]) with an \entryfn
creates an \emph{explicit fiber}. When the \entryfn passed to the constructor
of \fiber returns, the fiber terminates. The \entryfn for an explicit
fiber must return a non-empty \fiber object to designate the \emph{successor
fiber} for the current thread. The \entryfn for a thread need not return
a \fiber object because the entire thread is terminating.

5 Member functions of \fiber ([fiber.context.class]) can direct the calling
thread to \emph{suspend} the running fiber and \emph{resume} a designated
other fiber. Similarly, returning from the \entryfn for a fiber resumes
the designated successor fiber. This transition from one fiber to another is
a \emph{fiber context switch}.

6 Constructing an object of type \fiber with an \entryfn \emph{prepares} a
fiber, which can consume resources. A fiber can thus be in one of three
states: prepared, running or suspended.

7 When a thread first resumes a prepared fiber, that thread becomes the
...
The function \stdterm{\uncexs}{uncaught.exceptions}
...
\zs{Modify the last bullet 1.19 of \stdsection{14.6.2}{except.terminate} as
indicated:}

\begin{description}
    \item[---] when an exception is thrown from a
               coroutine \stdname{std::execution::task}{exec.task} which doesn’t support
               a \cpp{std::execution::set\_error\_t(std::exception\_ptr)}
               completion\replace{.}{, or}
\end{description}

\zs{Append after bullet 1.19 of \stdsection{14.6.2}{except.terminate}:}

\begin{description}
    \item[---] when an explicit fiber's \entryfn returns an empty \fiber
               object ([fiber.context]), or
    \item[---] when an explicit fiber's \entryfn terminates via an exception, or
    \item[---] when a non-empty \fiber object is destroyed, or
    \item[---] when a non-empty \fiber object is the target of an assignment.
\end{description}
...
\add{\fiber\cpp{::}\resumewith ([fiber.context.mem])}\\
...
2 An explicit fiber is prepared by passing an \entryfn to \fiber's
constructor. At the first call to one of the \anyresume member functions,
that \entryfn is invoked, and the fiber is running.

3 Every call to one of the \anyresume member functions on an accessible
non-empty \fiber object performs a fiber context switch:
\begin{itemize}
...
represent either an explicit fiber or a default fiber.}
...
%% 
%% Similarly, on every default fiber, the behaviour is equivalent to passing control through an
...
%% function for a default fiber does not deallocate the fiber's stack memory,
...
\begin{description}
    ...
    \item[---] \cpp{is\_invocable\_v<decay\_t<F>, fiber\_context\&\&>} is \true\\
               and \cpp{invoke\_result\_t<decay\_t<F>, fiber\_context\&\&>\&\&} denotes \fiber\cpp{\&\&}.
\end{description}
...
\begin{description}
    ...
    \item[---] Initializes \cpp{state} to prepare a fiber that will, when
               first resumed, invoke \cpp{entry\_copy}.
...
%% cross-ref P2019, which adds thread attribute to set size of thread/jthread.
...
1 \effects
If \cpp{empty()} is \false, \cpp{terminate} is invoked \xref{except.terminate}.
...
1 \effects
\begin{description}
    \item[---] If \cpp{empty()} is \false, \cpp{terminate} is invoked \xref{except.terminate}.
...
\begin{itemize}
    \item Invoking \resumewith performs a fiber context switch.
    \item The \emph{calling fiber} is the fiber invoking \resumewith.
    ...
    \item Because \resumewith suspends the calling fiber A, returning
          from \resumewith necessarily requires some other fiber B to perform a
          subsequent fiber context switch back to suspended fiber A.
          When \resumewith returns, the previous fiber is B.
          \tsnote{B can be other than the target fiber.}
    \item Let \cpp{previous} be the synthesized \fiber object representing B.
\end{itemize}

A fiber context switch to the target fiber requires the following steps:

\begin{enumerate}
    \item suspend the running fiber
    \item direct the current thread to make the target fiber \emph{current}:
          this is referred to as \emph{fiber-activation} of the target fiber
    \item execute \emph{internal-resume} on the target fiber (see below).
\end{enumerate}
...
Let \emph{internal-resume(\cpp{before})}, where \cpp{before} is a \fiber
object, denote the following sequence of steps in order.
\tsnote{\emph{internal-resume} is executed after performing \emph{fiber-activation}
of the target fiber.}

\begin{description}
    \item[---]
          If the new current fiber was suspended, that implies that at some earlier
          time, it called \resumewith on some
          non-empty \fiber object \cpp{other}. In that case:
        \begin{itemize}
            \item Return \cpp{before} from the call to \resumewith on \cpp{other}.
        \end{itemize}   
    \item[---] Otherwise:
        \begin{itemize}
            \item Evaluate
                  \cpp{invoke(entry\_copy, std::move(before))}
                  and let \cpp{successor} be the resulting \fiber
            \item destroy \cpp{entry\_copy}
            \item if \cpp{stack\_copy} and \cpp{deleter\_copy} exist:
                \begin{itemize}
                    \item evaluate \cpp{invoke(deleter\_copy, stack\_copy)}
                    \item destroy \cpp{deleter\_copy}
                \end{itemize}
            \item exit the running fiber
            \item reclaim implementation-provided resources
            \item perform \emph{fiber-activation} of \thefiber{\cpp{successor}}
            \item execute \emph{internal-resume(\cpp{fiber\_context()})}.
        \end{itemize}
\end{description}

1 \mandates
\cpp{is\_invocable\_v<decay\_t<Fn>, fiber\_context\&\&>} is \true
and \cpp{invoke\_result\_t<decay\_t<Fn>, fiber\_context\&\&>\&\&} denotes \fiber\cpp{\&\&}.

2 \precond
\cpp{can\_resume()} is \true.

3 \effects
\begin{description}
    \item[---] Resets \state so that \cpp{empty()} is \true.
    \item[---] Suspends the running fiber.
    \item[---] Associates the current thread as the target fiber's owning thread.
    \item[---] Performs \emph{fiber-activation} of the target fiber.
...
\begin{description}
    ...
    \item[---] If the previous fiber resumed the calling fiber by
          ...
          by \cpp{invoke(somefn, std::move(previous))}.
\end{description}
...
\begin{itemize}
    \item Any exception thrown by \cpp{invoke(somefn, std::move(previous))}.
\end{itemize}

\tsnote{\resumewith throws nothing before suspending the calling fiber and
ensuring \cpp{empty()} is \true.} 

6 \remarks

\begin{description}
    \item[---] \resumewith is vectorization-unsafe \xref{algorithms.parallel.defns}.
    \item[---] \resumewith may be recursively reentered \xref{reentrancy}.
\end{description}
...
1 \returns
\begin{description}
    \item[---] \false if \cpp{empty()} is \true
...
1 \effects
Exchanges the \cpp{state} of \this with that of \cpp{other}.
