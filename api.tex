...
\begin{description}
    ...
    \item[2)] takes invocable objects [func.def] as arguments. Both \cpp{entry}
              ...
              \cpp{fiber\_context(fiber\_context&&)}. This constructor template
...
\begin{description}
    \item[---] The entry-function \cpp{entry} is \emph{not} immediately entered.
              ...
              on construction, but \cpp{entry} is entered
    ...
    \item[---] The \cancelfn\xspace \cpp{cancel} is invoked by \anycancel (q.v.).
\end{description}
...
\requires
\begin{description}
    \item[---] \cpp{*this} represents a suspended fiber (\cpp{empty()} returns \cpp{false})
...
%             the suspended fiber is destroyed
    \item[---] \xtresumewith can
              ...
              \begin{itemize}
                  ...
                  \item the function \cpp{otherfn} passed to the other
                        fiber's call to \someresumewith
                        -- or some function called
                        by \cpp{otherfn} -- throws an exception
              \end{itemize}
...
\mbrhdr{cancel()}
cancels a suspended fiber by calling the \cancelfn passed to its constructor.

\begin{tabular}{ l l }
    \midrule

    \cpp{fiber\_context cancel\_from\_any\_thread() &&} \\
...
\effects
It is explicitly permitted to call \xtcancel on an empty \fiber instance,
which has no effect.

Otherwise, equivalent to:
\cpp{resume\_from\_any\_thread\_with(cancelfn)}
...
\returns
\begin{description}
    \item[---] If \cpp{*this} is empty, an empty \fiber instance.
    \item[---] Otherwise, a \fiber representing the immediately preceding
               fiber: the fiber that resumed this one, thereby suspending
               itself. \tsnote{If the immediately preceding fiber resumed this
               one by terminating itself, the returned \fiber will be empty.
               Otherwise, depending on the interaction of the \cancelfn and
               the code on the fiber represented by \cpp{*this}, it may or may
               not represent the fiber originally represented by \cpp{*this}
               at entry to \xtcancel.}
\end{description}

\except
\begin{description}
    \item[---] If \cpp{*this} is empty, none.
    \item[---] Otherwise, equivalent to \cpp{resume\_from\_any\_thread\_with(cancelfn)},
               where \cpp{cancelfn} is the \cancelfn passed to the constructor
               of \cpp{*this}.
\end{description}
...
\mbrhdr{can\_resume\_from\_this\_thread()}
query method
...
\mbrhdr{swap(fiber\_context\& other)}
