...
8 The exception with the most recently activated handler \add{in the current fiber ([intro.fibers])} that is
...
\begin{itemize}
    ...
    \item \resumewith suspends the calling fiber and synthesizes
          ...
          \cpp{caller} be that synthesized \fiber object.
    \item Let \cpp{previous} be the synthesized \fiber object representing the
          suspended previous fiber.
\end{itemize}

Given a non-empty \fiber object \cpp{after} representing a suspended
destination fiber:
...
\begin{description}
    \item[---]
          ...
          exposition-only \emph{internal-resume(\cpp{after}, \cpp{before})},
          where \cpp{before} is a \fiber object other than \cpp{after}, denote
          the following sequence of steps:
        \begin{itemize}
            \item Return \cpp{before} from \resumewith[other].
        \end{itemize}   
    \item[---] Otherwise, let \emph{internal-resume(\cpp{after}, \cpp{before})}
        ...
        \begin{itemize}
            ...
            \item destroy \cpp{after\_entry\_copy}, then
            \item if \cpp{after\_stack\_copy} and \cpp{after\_deleter\_copy} exist:
                \begin{itemize}
                    \item execute \cpp{invoke(after\_deleter\_copy, after\_stack\_copy)}, then
                    \item destroy \cpp{after\_deleter\_copy}, then
                \end{itemize}
            \item reclaim implementation-provided resources, then
            \item exit \thefiber{\cpp{after}}, then
            \item direct the current thread to resume \thefiber{\cpp{successor}}, then
            \item execute \emph{internal-resume(\cpp{successor}, \cpp{fiber\_context()})}.
        \end{itemize}
\end{description}
...
\begin{description}
    ...
    \item[---] Executes \emph{internal-resume(\cpp{target}, \cpp{returned})}.
\end{description}
