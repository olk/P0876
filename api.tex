...
the \fiber object). If \thefiber{the \fiber object} has not yet been entered,
...
\rSec3[fiber.context.pre]{Preamble}
...
\tsnote{\anyresume empties the object on which it is called. In order to
express the state change explicitly, these member functions are rvalue-reference
qualified. For this reason, no \fiber object ever represents the
...
all exceptions within the thread.
