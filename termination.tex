...
somehow arrange for access to its fiber's own \cpp{stop\_token}
or \cpp{stop\_source} to be able to query \cpp{stop\_requested()}. Moreover,
the fiber must be resumed \emph{after} the \reqstop call before it could
observe \cpp{stop\_requested()}. Even then, the \entryfn might not immediately
return.
