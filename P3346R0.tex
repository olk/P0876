...
3 Conceptually a \tptr\cpp{<T, Deleter>} instance stores a distinct
\cpp{unique\_ptr<T, Deleter>} instance for each consuming thread.
However, \cpp{Deleter} must be copyable so that \cpp{reset(T*)} can associate
...
2 \precond
\cpp{Deleter} meets the \emph{Cpp17DefaultConstructible} and
\emph{Cpp17CopyConstructible} requirements and such construction does not exit
via an exception.
\cpp{Deleter} meets the \emph{Cpp17CopyAssignable} requirements.
...
4 \postcond
\cpp{bool(*this) == false}
\tsnote{Conceptually the instance of \cpp{this->p} for each consuming thread
is initialized with \cpp{nullptr} and a copy of the stored deleter.}
...
%--------------------------------- move ctor ---------------------------------
\mbrhdr{thread\_specific\_ptr(thread\_specific\_ptr\&\& other)}
...
\begin{description}
    \item[---] Constructs a \cpp{thread\_specific\_ptr} object for storing a
               pointer to an object of type \cpp{T} specific to each thread.
               The stored deleter is move constructed from \cpp{other}'s deleter.
\end{description}
...
\tsnote{The requirement avoids the necessity for an implementation to track
all thread specific instances associated with each \tptr.}

\rSec4[thread.ptr.mem]{Members}

%------------------------------ move assignment ------------------------------
\mbrhdr{thread\_specific\_ptr\& operator=(thread\_specific\_ptr\&\& other)}

1 \constraints
\cpp{is\_move\_assignable\_v<Deleter> == true}

2 \precond
\cpp{Deleter} meets the \emph{Cpp17MoveAssignable} requirements and such
assignment does not exit via an exception.

3 \effects
\begin{description}
    \item[---] Equivalent to: \cpp{p = std::move(other.p);}
\end{description}
...
%--------------------------------- release() ---------------------------------
\mbrhdr{[[nodiscard]] unique\_ptr<T, Deleter> release() noexcept}

1 \effects
For the instance of \cpp{this->p} for the current thread, equivalent to:
\cpp{return std::move(p);}
...
%------------------------------- member swap() -------------------------------
\mbrhdr{void swap(thread\_specific\_ptr\& other) noexcept}

1 \effects
Swaps the pointer associated with the calling thread for \this with the
pointer associated with the calling thread for \cpp{other}.

%------------------------------- friend swap() -------------------------------
\rSec4[thread.ptr.special]{Specialized algorithms}
\mbrhdr{friend void swap(thread\_specific\_ptr\& lhs, thread\_specific\_ptr\& rhs) noexcept}
