...
1 \mandates
\begin{description}
    \item[---] \cpp{delete this->get()} is well-formed.
\end{description}
...
\begin{description}
    \item[---] Constructs a \cpp{thread\_specific\_ptr} object for storing a
               ...
               The default \cpp{delete}-based cleanup function will be used to
               destroy any thread-local objects when \cpp{reset()} is called,
               or the thread exits.
\end{description}

3 \remarks
The initial value associated with an instance of \tptr is \cpp{nullptr} for
each thread.
...
\mbrhdr{explicit thread\_specific\_ptr(void (*cleanup\_function)(T*))}

1 \mandates
\begin{description}
    \item[---] \cpp{cleanup\_function(this->get())} is well-formed.
\end{description}

2 \requires
\begin{description}
    \item[---] \cpp{cleanup\_function(this->get())} does not throw. 
\end{description}

3 \effects
\begin{description}
    \item[---] Constructs a \tptr object for storing a pointer to an object of
               type \cpp{T} specific to each thread. The supplied
               \cpp{cleanup\_function} will be used to destroy any
               thread-local objects when \cpp{reset()} is called, or the
               thread exits.
\end{description}
...
\mbrhdr{void reset(T* new\_value=nullptr)}

1 \effects
\begin{description}
    \item[---] If \cpp{this->get() \!= new\_value \&\& this->get() \!= nullptr},
               invokes \cpp{delete this->get()} or\\
               \cpp{cleanup\_function(this->get())} as appropriate.
